...
static int zmk_peripheral_ble_report_event(const struct zmk_split_transport_peripheral_event *ev) {
    ...
    case ZMK_SPLIT_TRANSPORT_PERIPHERAL_EVENT_TYPE_KEY_POSITION:
        if (ev->data.key_position.pressed) {
            zmk_split_bt_position_pressed(ev->data.key_position.position);
        } else {
            zmk_split_bt_position_released(ev->data.key_position.position);
        }
        break;
#if ZMK_KEYMAP_HAS_SENSORS
...
static ssize_t split_svc_run_behavior(struct bt_conn *conn, const struct bt_gatt_attr *attrs,
    ...
    if ((end_addr > sizeof(struct zmk_split_run_behavior_data)) &&
        ...
        const size_t payload_dev_size = sizeof(cmd.data.trigger_behavior.behavior_dev);
        if (strlcpy(cmd.data.trigger_behavior.behavior_dev, payload->behavior_dev,
                    payload_dev_size) >= payload_dev_size) {
            LOG_ERR("Truncated behavior label %s to %s before invoking peripheral behavior",
                    payload->behavior_dev, cmd.data.trigger_behavior.behavior_dev);
        }

        LOG_DBG("%s with params %d %d: pressed? %d", cmd.data.trigger_behavior.behavior_dev,
                cmd.data.trigger_behavior.param1, cmd.data.trigger_behavior.param2,
                cmd.data.trigger_behavior.state);
