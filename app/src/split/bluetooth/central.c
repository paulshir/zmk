        ...
        for (int j = 0; j < 8; j++) {
            if (slot->position_state[i] & BIT(j)) {
                ...
                struct peripheral_event_wrapper ev = {
                    .source = index,
                    .event = {.type = ZMK_SPLIT_TRANSPORT_PERIPHERAL_EVENT_TYPE_KEY_POSITION,
                              .data = {.key_position = {
                                           .position = position,
                                           .pressed = false,
                                       }}}};
    ...
    while (k_msgq_get(&zmk_split_central_split_run_msgq, &payload_wrapper, K_NO_WAIT) == 0) {
        ...
        case ZMK_SPLIT_TRANSPORT_CENTRAL_CMD_TYPE_TRIGGER_BEHAVIOR: {
            ...
            struct zmk_split_run_behavior_payload payload = {
                .data = {
                    .param1 = payload_wrapper.cmd.data.trigger_behavior.param1,
                    .param2 = payload_wrapper.cmd.data.trigger_behavior.param2,
                    .position = payload_wrapper.cmd.data.trigger_behavior.position,
                    .source = payload_wrapper.cmd.data.trigger_behavior.event_source,
                    .state = payload_wrapper.cmd.data.trigger_behavior.state ? 1 : 0,
                }};
            const size_t payload_dev_size = sizeof(payload.behavior_dev);
            if (strlcpy(payload.behavior_dev,
                        payload_wrapper.cmd.data.trigger_behavior.behavior_dev,
...
void peripheral_event_work_callback(struct k_work *work) {
    ...
    while (k_msgq_get(&peripheral_event_msgq, &ev, K_NO_WAIT) == 0) {
        LOG_DBG("Trigger key position state change for %d", ev.event.data.key_position.position);
