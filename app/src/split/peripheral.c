...
int zmk_split_transport_peripheral_command_handler(
    ...
    case ZMK_SPLIT_TRANSPORT_CENTRAL_CMD_TYPE_TRIGGER_BEHAVIOR: {
        struct zmk_behavior_binding binding = {
            .param1 = cmd.data.trigger_behavior.param1,
            .param2 = cmd.data.trigger_behavior.param2,
            .behavior_dev = cmd.data.trigger_behavior.behavior_dev,
        };
        LOG_DBG("%s with params %d %d: pressed? %d", binding.behavior_dev, binding.param1,
                binding.param2, cmd.data.trigger_behavior.state);
        struct zmk_behavior_binding_event event = {.position = cmd.data.trigger_behavior.position,
                                                   .timestamp = k_uptime_get()};
        int err;
        if (cmd.data.trigger_behavior.state > 0) {
...
int split_peripheral_listener(const zmk_event_t *eh) {
    ...
    if ((pos_ev = as_zmk_position_state_changed(eh)) != NULL) {
        struct zmk_split_transport_peripheral_event ev = {
            .type = ZMK_SPLIT_TRANSPORT_PERIPHERAL_EVENT_TYPE_KEY_POSITION,
            .data = {.key_position = {
